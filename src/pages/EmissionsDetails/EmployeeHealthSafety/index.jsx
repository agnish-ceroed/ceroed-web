...
const EmployeeHealthSafety = (props) => {
  ...
  useEffect(() => {
    if (deleteEmissionData.status === STATUS.SUCCESS) {
      enqueueSnackbar(
        "Employee health safety incident record deleted successfully",
        {
          variant: "success",
        }
      );
  ...
  const onUpdatePurchasedElectricity = () => {
    navigate(
      `/emissions/edit/employee_health_safety_incident_record/${emissionId}`
    );
  };
        ...
        <Box className={classes.topContainer}>
          ...
          >
            <Grid item container direction={"column"} xs={10}>
              ...
              <Typography className={classes.previewItem}>
                Department: {emissionData.department}
              </Typography>
              <Typography className={classes.previewItem}>
