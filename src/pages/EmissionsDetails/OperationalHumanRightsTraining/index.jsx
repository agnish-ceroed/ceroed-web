...
const HumanRightsTraining = (props) => {
  ...
  useEffect(() => {
    if (deleteEmissionData.status === STATUS.SUCCESS) {
      enqueueSnackbar(
        "Operational Human Rights Training deleted successfully",
        {
          variant: "success",
        }
      );
  ...
  const onUpdatePurchasedElectricity = () => {
    navigate(`/emissions/edit/operational_human_rights_training/${emissionId}`);
  };
      ...
      <Box className={classes.innerContainer}>
        <Box className={classes.header}>
          <Typography variant="h6" component="div">
            Operational Human Rights Training
          </Typography>
          <Box>
            <CeroButton
              variant="outlined"
