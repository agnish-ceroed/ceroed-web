...
const SocialEngagementHumanRightsTraining = (props) => {
  ...
  useEffect(() => {
    if (deleteEmissionData.status === STATUS.SUCCESS) {
      enqueueSnackbar(
        "Social Engagement Human Rights Training deleted successfully",
        {
          variant: "success",
        }
      );
  ...
  const onUpdatePurchasedElectricity = () => {
    navigate(
      `/emissions/edit/social_engagement_human_rights_training/${emissionId}`
    );
  };
      ...
      <Box className={classes.innerContainer}>
        <Box className={classes.header}>
          <Typography variant="h6" component="div">
            Social Engagement Human Rights Training
          </Typography>
          <Box>
            <CeroButton
              variant="outlined"
...
export default SocialEngagementHumanRightsTraining;
